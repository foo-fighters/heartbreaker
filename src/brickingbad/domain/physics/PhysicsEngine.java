...
public class PhysicsEngine implements Runnable {
  ...
  private static boolean mixedColliding(GameObject circle, GameObject rect) {
    ...
    if(line_x1.isVectorLower(circle.getPosition())) {
      if(line_y1.isVectorLower(circle.getPosition())) {
        circle.setReflectionDirection(Direction.UP_LEFT);
        return Math.hypot(circle_x - point_x1y1.getX(), circle_y - point_x1y1.getY()) < radius;
      }else if(line_y2.isVectorHigher(circle.getPosition())) {
        circle.setReflectionDirection(Direction.DOWN_LEFT);
        return Math.hypot(circle_x - point_x1y2.getX(), circle_y - point_x1y2.getY()) < radius;
      }else {
        circle.setReflectionDirection(Direction.LEFT);
        return line_x1.distanceToLine(circle.getPosition()) < radius;
      }
    }else if(line_x2.isVectorHigher(circle.getPosition())) {
      if(line_y1.isVectorLower(circle.getPosition())) {
        circle.setReflectionDirection(Direction.UP_RIGHT);
        return Math.hypot(circle_x - point_x2y1.getX(), circle_y - point_x2y1.getY()) < radius;
      }else if(line_y2.isVectorHigher(circle.getPosition())) {
        circle.setReflectionDirection(Direction.DOWN_RIGHT);
        return Math.hypot(circle_x - point_x2y2.getX(), circle_y - point_x2y2.getY()) < radius;
      }else {
        circle.setReflectionDirection(Direction.RIGHT);
        return line_x2.distanceToLine(circle.getPosition()) < radius;
      }
    }else {
      if(line_y1.isVectorLower(circle.getPosition())) {
        circle.setReflectionDirection(Direction.UP);
        return line_y1.distanceToLine(circle.getPosition()) < radius;
      }else if(line_y2.isVectorHigher(circle.getPosition())) {
        circle.setReflectionDirection(Direction.DOWN);
        return line_y2.distanceToLine(circle.getPosition()) < radius;
      }else {
        circle.setReflectionDirection(Direction.DOWN);
        return true;
      }
